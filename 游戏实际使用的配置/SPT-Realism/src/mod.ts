...
export class Main implements IPreSptLoadMod, IPostDBLoadMod, IPostSptLoadMod {
    ...
    public preSptLoad(container: DependencyContainer): void {
        ...
        const traderPurchasePersisterService = container.resolve<TraderPurchasePersisterService>("TraderPurchasePersisterService");
        ...
        const traderRefresh = new TraderRefresh(logger, mathUtil, timeUtil, databaseService, profileHelper, assortHelper, paymentHelper, ragfairAssortGenerator, ragfairOfferGenerator,
            traderAssortService, localisationService, traderPurchasePersisterService, traderHelper, fenceService, configServer, cloner);
        ...
        container.afterResolution("TraderAssortHelper", (_t, result: TraderAssortHelper) => {
            result.resetExpiredTrader = (trader: ITrader): void => {
                return traderRefresh.myResetExpiredTrader(trader);
            }
        }, { frequency: "Always" });
            ...
            [
                {
                    ...
                    action: async (url, info, sessionID, output) => {
                        ...
                        const seasonalEventConfig = container.resolve<ConfigServer>("ConfigServer").getConfig<ISeasonalEventConfig>(ConfigTypes.SEASONAL_EVENT);
                        ...
                        try {
                            ...
                            this.checkForSeasonalEvents(logger, seasonalEventsService, seasonalEventConfig, weatherConfig);
    ...
    public postDBLoad(container: DependencyContainer): void {
        ...
        this.checkForSeasonalEvents(logger, seasonalEventsService, seasonalEventConfig, weatherConfig, true);
    ...
    private modifyMapLoot(locationConfig: ILocationConfig, map: string, raidData: IEndLocalRaidRequestData, pmcData: IPmcData, profileId: string, utils: Utils, logger: ILogger) {
        ...
        const looseModifier = utils.clampNumber(lootXp * 0.00059, 0, 0.60); //极限搜刮(1000XP)每局-0.59，5局触底(20%)，6局恢复
        const staticModifier = utils.clampNumber(lootXp * 0.00008, 0, 0.15); //极限搜刮(1000XP)每局-0.08，10局触底(20%)，12局恢复
        const looseLootRegenRate = 0.148;
        const staticLootRegenRate = 0.020;
        ...
        const minLooseLoot = originalLooseMapModi * 0.2;
        const minStaticLoot = originalStaticMapModi * 0.2;
